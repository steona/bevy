...
public class NoteEditActivity extends Activity {
	...
	private TextView mRepeatUntilDateDisplay;
	private Long mRowId;
	...
	private Button mRepeatUntilDate;
	static final int DATE_DIALOG_ID = 0;
	static final int DATE_REPEAT_DIALOG_ID = 1;
	private long id = -1;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		mRepeatUntilDate = (Button) findViewById(R.id.repeatUntil);
		mRepeatUntilDateDisplay = (TextView) findViewById(R.id.repeatdate);
		...
		if (extra != null) {
			...
			if (id >= 0) {
				...
				Cursor c = db.query("entries", null, "id = " + id, null, null,
						null, null);
				if (c.moveToFirst()) {
					String title = c.getString(c.getColumnIndex("title"));
					String createdDate = c.getString(c
							.getColumnIndex("date_added"));
					String body = c.getString(c.getColumnIndex("entry"));
					String lastModified = c.getString(c
							.getColumnIndex("last_modified"));
					if (title != null) {
		...
		mRepeatUntilDate.setOnClickListener(new View.OnClickListener() {
			public void onClick(View v) {
				showDialog(DATE_REPEAT_DIALOG_ID);
			}
		});
		...
		mRowId = savedInstanceState != null ? savedInstanceState
				.getLong(PersonalDiaryDB.KEY_ROWID) : null;
		if (mRowId == null) {
	...
	protected Dialog onCreateDialog(int id) {
		...
		case DATE_DIALOG_ID:
			return new DatePickerDialog(this, mDateSetListener, mYear, mMonth,
					mDay);
		case DATE_REPEAT_DIALOG_ID:
			return new DatePickerDialog(this, mRepeatDateSetListener, mYear,
					mMonth, mDay);
	...
	private void updateDisplay(int display) {
		mTimestamp = new StringBuilder().append(mYear).append("-");
		...
		if (display == DATE_DIALOG_ID) {
			mDateDisplay.setText(mTimestamp);
		}
		if (display == DATE_REPEAT_DIALOG_ID) {
			mRepeatUntilDateDisplay.setText(mTimestamp);
		}
	}
	...
	private void saveState() {
		...
		String date = mDateDisplay.getText().toString();// mDateText.getText().toString();
		String repeatUntilDate = mRepeatUntilDateDisplay.getText().toString();
		String repeatEvery = mRepeatText.getText().toString();

		if (repeatEvery != null && !repeatEvery.equalsIgnoreCase("")) {
			try {
				int iRepeatEvery = Integer.parseInt(repeatEvery);
				SimpleDateFormat sdf = new SimpleDateFormat(
						"yyyy-MM-dd HH:mm:ss");

				Date eventDate = sdf.parse(date);
				Date endDate = sdf.parse(repeatUntilDate);

				while (endDate.after(eventDate) || endDate.equals(eventDate)) {

					if (id <= 0) {
						long id = mDbHelper.createNote(title, body, sdf
								.format(eventDate));
						if (id > 0) {
							mRowId = id;
						}
					...
					c.setTime(eventDate);
					c.add(Calendar.DATE, iRepeatEvery);
					eventDate = c.getTime();
				}
				return;
			} catch (ParseException e) {
	...
	private DatePickerDialog.OnDateSetListener mRepeatDateSetListener = new DatePickerDialog.OnDateSetListener() {
		...
		public void onDateSet(DatePicker view, int year, int monthOfYear,
			...
			mMonth = monthOfYear;
			mDay = dayOfMonth;
			updateDisplay(DATE_REPEAT_DIALOG_ID);
		}
	};
